...
import useStore, { useNodeData } from "../store";
...
export default function FloatInput({ path }: FloatInputProps) {
  ...
  const fieldData = useNodeData(path) as
    | { type: string; default_value?: number }
    | undefined;
  const currentValue = useNodeData([...path, "value"]) as number | undefined;
  ...
  useEffect(() => {
    ...
    if (isConnected && currentValue !== undefined) {
      setValue(currentValue);
    }
  ...
  const handleValueChange = (newValue: number | undefined) => {
    setValue(newValue);
    // Update the store with the new value using the store action
    updateNodeData([...path, "value"], newValue ?? 0);
  };
  ...
  const handleBlur = () => {
    if (value === undefined) {
      ...
      setValue(defaultVal);
      updateNodeData([...path, "value"], defaultVal);
    }
  };
